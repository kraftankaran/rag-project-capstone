...
// NEW: splits text and wraps every match of `term` in a yellow <mark>
...
export default function Search() {
  ...
  const [isSearching, setIsSearching] = useState(false);
  const [results, setResults] = useState(null);
  // NEW: snapshot of the query at the time Search was submitted,
  // so highlights don't shift if the user keeps typing afterward
  ...
  const handleSearch = async (e) => {
    ...
    setIsSearching(true);
    setResults(null);
    setSubmittedQuery(query.trim()); // NEW
              ...
              {results.map((result, i) => (
                ...
                >
                  <div className="card-content" style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                        {/* MODIFIED: wrap title text in HighlightedText */}
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--primary)', wordBreak: 'break-all' }}>
                          <HighlightedText text={result.title} term={submittedQuery} />
                        </h3>
                        <span style={{ fontSize: '0.75rem', background: 'var(--muted)', padding: '4px 10px', borderRadius: '12px', color: 'var(--accent)', fontWeight: 600 }}>
                          Score: {result.relevance > 0 ? result.relevance.toFixed(3) : "N/A"}
                        </span>
                      ...
                      <p style={{ color: 'var(--foreground)', lineHeight: 1.6, fontSize: '0.95rem' }}>
                        <HighlightedText text={result.snippet} term={submittedQuery} />
                      </p>
                    </div>
                    <ChevronRight size={20} color="var(--muted-foreground)" style={{ alignSelf: 'center', flexShrink: 0 }} />
                  </div>
